...
def _start_process(pyngrok_config):
    ...
    start = [pyngrok_config.ngrok_path, "start", "--log=stdout"]
    if pyngrok_config.start_all:
        logger.info("Starting ngrok in all tunnels")
        start.append("--all")
    if pyngrok_config.config_path:
    ...
    logger.info("ngrok process starting with PID: {}".format(proc.pid))
